import os

def main():
    brain_dir = r"C:\Users\ashok\.gemini\antigravity\brain"
    past_session = "9a68881f-9f42-497a-9b69-5e622236c737"
    tasks_dir = os.path.join(brain_dir, past_session, ".system_generated", "tasks")
    
    # List of logs to read
    logs_to_read = [
        "task-236.log", "task-308.log", "task-319.log", "task-338.log", 
        "task-347.log", "task-385.log", "task-399.log", "task-407.log"
    ]
    
    for log_name in logs_to_read:
        path = os.path.join(tasks_dir, log_name)
        if os.path.exists(path):
            print(f"\n==================== {log_name} ====================")
            with open(path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
                # print last 1000 characters or full
                if len(content) > 1000:
                    print(content[-1000:])
                else:
                    print(content)
        else:
            print(f"Log {log_name} does not exist.")

if __name__ == '__main__':
    main()
