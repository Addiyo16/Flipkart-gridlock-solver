# test_notebook_logic.py - Train the notebook's model pipeline (Ensemble + Lags + TE inside loop)
import os
import gc
import numpy as np
import pandas as pd
...
def main():
    print("--- Training Notebook Logic (Ensemble + Lags + TE) ---")
    ...
    df['geo_prefix5_hour'] = df['geohash_5'].astype(str) + '_' + df['hour'].astype(str)
    df['weather_time'] = df['Weather'].astype(str) + '_' + df['timestamp'].astype(str)
    ...
    for offset, col_name in zip(lag_offsets, lag_cols):
        mapped = [d48_dict.get((geo, max(0, min(1425, m + offset))), np.nan) 
                  for geo, m in zip(df.loc[is_d49, 'geohash'], df.loc[is_d49, 'minutes'])]
        df.loc[is_d49, col_name] = mapped
        
    df.loc[is_d49, 'geo_mean_yesterday'] = df.loc[is_d49, 'geohash'].map(d48_geo_mean)
    ...
    lag_array_d49 = df.loc[is_d49, lag_cols].values
    df.loc[is_d49, 'lag_mean'] = np.nanmean(lag_array_d49, axis=1)
    df.loc[is_d49, 'lag_max'] = np.nanmax(lag_array_d49, axis=1)
    df.loc[is_d49, 'lag_min'] = np.nanmin(lag_array_d49, axis=1)
    df.loc[is_d49, 'lag_std'] = np.nanstd(lag_array_d49, axis=1)
    ...
    cat_cols = ['RoadType', 'LargeVehicles', 'Landmarks', 'Weather', 'geohash', 'geohash_3', 'geohash_4', 'geohash_5', 'timestamp',
                'geo_time', 'geo_hour', 'geo_prefix5_hour', 'weather_time']
    for col in cat_cols:
        le = LabelEncoder()
    ...
    oof_lgb = np.zeros(len(X_train_df))
    oof_xgb = np.zeros(len(X_train_df))
    oof_cat = np.zeros(len(X_train_df))
    
    preds_lgb = np.zeros(len(X_test_df))
    preds_xgb = np.zeros(len(X_test_df))
    preds_cat = np.zeros(len(X_test_df))
    
    te_cols = ['geohash', 'timestamp', 'geo_time', 'geo_hour', 'geo_prefix5_hour', 'weather_time']
    smoothing = 20
    
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_df, y_train)):
        print(f"\n--- Fold {fold+1} / {n_splits} ---")
        X_tr = X_train_df.iloc[train_idx].copy()
        X_va = X_train_df.iloc[val_idx].copy()
        y_tr = y_train[train_idx]
        y_va = y_train[val_idx]
        
        X_te_fold = X_test_df.copy()
        
        # Target Encoding computed strictly on training folds to prevent target data leakage
        X_tr['demand'] = y_tr
        for col in te_cols:
            global_mean = y_tr.mean()
            stats = X_tr.groupby(col)['demand'].agg(['count', 'mean'])
            ...
            X_tr[col + '_te'] = X_tr[col].map(smooth_mean).fillna(global_mean)
            X_va[col + '_te'] = X_va[col].map(smooth_mean).fillna(global_mean)
            X_te_fold[col + '_te'] = X_te_fold[col].map(smooth_mean).fillna(global_mean)
            
        X_tr = X_tr.drop(columns=['demand'])
        features = [c for c in X_tr.columns if c not in ['Index', 'demand']]
        
        X_tr_np = X_tr[features].values
        X_va_np = X_va[features].values
        X_te_np = X_te_fold[features].values
        ...
        model_lgb.fit(X_tr_np, y_tr, eval_set=[(X_va_np, y_va)], callbacks=[lgb.early_stopping(50, verbose=False)])
        oof_lgb[val_idx] = model_lgb.predict(X_va_np)
        preds_lgb += model_lgb.predict(X_te_np) / n_splits
        ...
        model_xgb.fit(X_tr_np, y_tr, eval_set=[(X_va_np, y_va)], verbose=False)
        oof_xgb[val_idx] = model_xgb.predict(X_va_np)
        preds_xgb += model_xgb.predict(X_te_np) / n_splits
        ...
        model_cat.fit(X_tr_np, y_tr, eval_set=(X_va_np, y_va))
        oof_cat[val_idx] = model_cat.predict(X_va_np)
        preds_cat += model_cat.predict(X_te_np) / n_splits
    ...
    print("\n--- Day 49 Validation Summary (Notebook Logic - Lags + TE) ---")
    ...
    submission_path = r"C:\Users\ashok\.gemini\antigravity\scratch\traffic-prediction\submission_notebook.csv"
