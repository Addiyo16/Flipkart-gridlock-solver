...
def main():
    print("--- Traffic Demand Prediction: Log-Ratio Ensemble ---")
    ...
    test['is_day_49'] = 1  # test set is all Day 49
    ...
    # Compute geohash mean temperature from train set
    ...
    for col in cat_cols:
        le = LabelEncoder()
        # Combine to fit_transform on all categories
    ...
    print("\n--- Out-of-Fold Day 49 Validation R2 ---")
    print(f"  LightGBM R2: {r2_d49_lgb:.5f}")
    print(f"  XGBoost R2:  {r2_d49_xgb:.5f}")
    print(f"  CatBoost R2: {r2_d49_cat:.5f}")
    print(f"  Ensemble Blend R2: {r2_d49_blend:.5f} (HackerEarth Score Estimate: {100 * r2_d49_blend:.3f})")
    ...
    scratch_sub_path = r"C:\Users\ashok\.gemini\antigravity\scratch\submission_final.csv"
    root_sub_path = r"C:\Users\ashok\.gemini\submission_final.csv"
    
    submission.to_csv(scratch_sub_path, index=False)
    submission.to_csv(root_sub_path, index=False)
    
    print(f"\nSaved submission to:\n  - {scratch_sub_path}\n  - {root_sub_path}")
    print("Dimensions of submission:", submission.shape)
    print("Head of submission:\n", submission.head(10))
    print("\nLog-Ratio Ensemble training and inference completed successfully!")
